...
export async function generateTranscribe(audioFilePath: string, outputJsonFilePath: string): Promise<{ languageCode: string }> {
    ...
    const failed = fileResults.failed.find((f: { file_name: string, status: string, error_message?: string }) => f.file_name === baseName);
    if (failed) {
        throw new Error(`Failed to transcribe ${failed.file_name}: ${failed.error_message}`);
    }

    const successful = fileResults.successful.find((f: { file_name: string, status: string }) => f.file_name === baseName);
    if (!successful) {
