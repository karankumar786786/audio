...
export class UserController {
    ...
    getUserHistory = asyncHandler(async (req: Request, res: Response) => {
        const userId = (req as any).user.id;
        const params: PaginationParams = parsePagination(req.query);
        const result: PaginatedResult<HistoryEvent> = await this.userService.getHistory(userId, params.limit, (params.page - 1) * params.limit);
        return new ApiResponse<PaginatedResult<HistoryEvent>>(200, "User history fetched", result).send(res);
    });
    ...
    createUserPlaylist = asyncHandler(async (req: Request, res: Response) => {
        ...
        const playlist: UserPlaylistSchema = await this.userService.createUserPlaylist(requestBody);
