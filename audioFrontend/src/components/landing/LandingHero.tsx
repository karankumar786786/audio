...
import { Sparkles, ArrowRight, Music2, Waves, Radio } from "lucide-react";
...
function useSimLoop(total: number, intervalMs: number) {
  ...
  useEffect(() => {
    const lyricId = setInterval(() => setIndex((i) => (i + 1) % total), intervalMs);
    ...
    return () => { clearInterval(lyricId); clearInterval(progId); };
  }, [total, intervalMs]);
...
export function LandingHero() {
  const { index: activeIndex, progress, elapsed } = useSimLoop(lyrics.length, INTERVAL_MS);
    ...
    <main className="max-w-6xl mx-auto px-6 pt-28 md:pt-36 pb-16 relative z-10 flex flex-col items-center">
      ...
      >
        <div
          ...
          style={{
            background: "linear-gradient(160deg, #0c0e0c 0%, #080a08 60%, #0a0e0a 100%)",
        ...
        >
          ...
          />
          {/* Side glows */}
          <div className="absolute -left-20 top-1/2 -translate-y-1/2 w-48 h-48 rounded-full pointer-events-none"
            style={{ background: "radial-gradient(circle, rgba(120,240,142,0.07), transparent 70%)" }} />
          <div className="absolute -right-20 top-1/2 -translate-y-1/2 w-48 h-48 rounded-full pointer-events-none"
            style={{ background: "radial-gradient(circle, rgba(52,211,153,0.05), transparent 70%)" }} />

          {/* Floating particles */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {Array.from({ length: 14 }).map((_, i) => (
              <Particle key={i} delay={i * 0.35} />
            ))}
          </div>
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <div
                ...
                style={{
                  background: "linear-gradient(135deg, rgba(120,240,142,0.15), rgba(52,211,153,0.08))",
              ...
              <div>
                <p className="text-sm font-black text-white/90 tracking-tight">Now Playing</p>
                <p className="text-[10px] text-zinc-500 font-medium tracking-wider uppercase">One Melody Stream</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">320kbps FLAC</span>
              </div>
              ...
              >
                ...
                <span className="text-[9px] font-bold text-primary uppercase tracking-widest">Live Sync</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-5">
            ...
            {/* LEFT: Lyrics */}
            <div className="md:col-span-3 relative px-8 py-10 overflow-hidden" style={{ minHeight: "260px" }}>
              {/* Top fade */}
              <div className="absolute top-0 left-0 right-0 h-12 z-20 pointer-events-none"
                style={{ background: "linear-gradient(to bottom, rgba(8,10,8,0.98), transparent)" }} />
              {/* Bottom fade */}
              <div className="absolute bottom-0 left-0 right-0 h-12 z-20 pointer-events-none"
                style={{ background: "linear-gradient(to top, rgba(8,10,8,0.98), transparent)" }} />

              <div className="relative z-10 flex flex-col justify-center gap-3.5 h-full text-left">
                {offsets.map((offset) => {
                  const idx = (activeIndex + offset + lyrics.length) % lyrics.length;
                  const line = lyrics[idx];
                  const isActive = offset === 0;
                  const dist = Math.abs(offset);

                  return (
                    <AnimatePresence mode="wait" key={`${offset}-${activeIndex}`}>
                      <motion.div
                        key={`${idx}-${offset}`}
                      ...
                      >
                        {isActive ? (
                          <p
                            ...
                            style={{
                              background: "linear-gradient(90deg, #fff 30%, rgba(120,240,142,0.9) 100%)",
                              WebkitBackgroundClip: "text",
                              WebkitTextFillColor: "transparent",
                              backgroundClip: "text",
                              textShadow: "none",
                              filter: "drop-shadow(0 0 20px rgba(120,240,142,0.35))",
                            }}
                          >
                            {line}
                          </p>
                        ) : (
                          <p className="font-medium text-lg md:text-xl leading-tight text-zinc-700">
                            {line}
                          </p>
                        )}
                      </motion.div>
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Spectrum Analyzer</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label}
                    ...
                    <p className="text-[7px] text-zinc-600 uppercase tracking-wider font-bold">{label}</p>
                    <p className="text-[11px] font-black text-white/80 mt-0.5">{value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              <motion.div
                className="absolute h-full rounded-full"
                style={{ background: "linear-gradient(90deg, var(--primary) 0%, #34d399 100%)", boxShadow: "0 0 8px rgba(120,240,142,0.5)" }}
