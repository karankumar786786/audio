...
import { Play, Pause, SkipForward, SkipBack, Volume2, VolumeX, Music, ChevronDown, Activity, Repeat1, Layers, Clock } from "lucide-react";
...
export function ShakaMusicPlayer() {
  ...
  const syncTracks = useCallback(() => {
    ...
    const unique = tracks
      .filter((t: any, idx: number, self: any[]) => self.findIndex((x) => x.bandwidth === t.bandwidth) === idx)
  ...
  if (!currentSong) {
    return (
      <div className="w-[360px] bg-zinc-950 border-l border-white/5 flex flex-col items-center justify-center p-10 text-center flex-none">
        ...
        <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest italic opacity-50">Select a frequency to synchronize</p>
      </div>
    );
  }
    ...
    <div className="w-[360px] bg-zinc-950 border-l border-white/5 flex flex-col h-screen overflow-hidden flex-none relative z-50">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-30">
        <div 
          ...
          style={{ backgroundImage: `url(${currentSong.posterUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
        />
      ...
      <div className="flex-none p-8 pt-12 text-center relative z-10">
        ...
        >
          <img src={currentSong.posterUrl} className="w-full h-full object-cover" alt={currentSong.title} />
        </motion.div>
      </div>
                ...
                {currentCaption.words.length > 0 ? (
                  currentCaption.words.map((word, idx) => {
                    ...
                    return (
                      <motion.span
                        key={idx}
                        animate={{ color: isActive ? "#ffffff" : "rgba(255,255,255,0.2)", scale: isActive ? 1.05 : 1 }}
                  ...
                  <span className="text-2xl font-black italic tracking-tighter text-white/90">{currentCaption.transcript}</span>
                )}
             </div>
           ) : (
      ...
      <div className="flex-none p-8 pb-10 space-y-8 relative z-10 bg-gradient-to-t from-zinc-950 via-zinc-950 to-transparent">
        ...
        <div className="space-y-1">
          <h2 className="text-xl font-black text-zinc-100 italic uppercase tracking-tighter truncate">{currentSong.title}</h2>
          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] italic opacity-80">{currentSong.artistName}</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative h-1 bg-zinc-900 rounded-full overflow-hidden group cursor-pointer">
            <div className="absolute top-0 left-0 h-full bg-zinc-800" style={{ width: `${(buffered / duration) * 100}%` }} />
            <div className="absolute top-0 left-0 h-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]" style={{ width: `${(localTime / duration) * 100}%` }} />
          </div>
        </div>

        {/* Core Buttons */}
        <div className="flex items-center justify-between">
           <button onClick={() => playerActions.toggleRepeat()} className={`p-2 transition-colors ${repeatMode !== "none" ? "text-indigo-400" : "text-zinc-600 hover:text-white"}`}>
              <Repeat1 size={20} />
           </button>
           ...
           <button onClick={() => setShowQualityMenu(!showQualityMenu)} className="p-2 text-zinc-600 hover:text-white">
              <Layers size={20} />
           </button>
        </div>
